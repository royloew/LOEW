...
` +
  ...
  const delta =
    currentWeightKg != null ? (currentWeightKg - target).toFixed(1) : null;

  const deltaLine =
    delta != null
      ? `זה ירידה של כ-${delta} ק״ג מהמשקל הנוכחי שלך.
`
      : "";

  return {
    reply:
      `מעולה. שמרתי מטרה: להגיע ל-${target} ק״ג תוך ${weeks} שבועות.
` +
      deltaLine +
      "סיימנו אונבורדינג 🎉",
    onboarding: true,
