...
async function init() {
  ...
  // לוג אינג'סט (מתי עשינו עדכון מסטרבה לאחרונה)
  await run(`
    CREATE TABLE IF NOT EXISTS strava_ingest_log (
      user_id       TEXT PRIMARY KEY,
...
export async function createDbImpl() {
  ...
  async function setLastIngestAtNow(userId) {
    const now = Math.floor(Date.now() / 1000);
    await run(
      `INSERT INTO strava_ingest_log (user_id, last_ingest_at)
       VALUES (?, ?)
       ON CONFLICT(user_id) DO UPDATE SET
  ...
  async function getLatestActivityStartSec(userId) {
    const row = await get(
  ...
  async function computeVolumeAndSummaryFromDb(userId) {
    ...
    if (!rows.length) {
    await setLastIngestAtNow(userId);
